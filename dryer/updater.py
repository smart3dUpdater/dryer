...
def check_health(containers=['all']):
    ...
    try:
        ...
        if output.get('error') == '':
            ...
            debug_print('Checking health of containers ...', end='')
            print_acert()
            return dic
    except Exception as error:
        print_error('Fail')
    ...
    for attempts in range(5):
        try:
            ...
            if file_integrity[0] != 'error':
                ...
                debug_print('Up services ...', end='')
                print_acert()
                bash_command('docker system prune -f')
                debug_print('Purge system ...', end='')
                print_acert()
                if file == BACKUP_PATH:
